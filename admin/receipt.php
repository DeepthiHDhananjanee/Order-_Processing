...
<body>
	...
	<div id="rightcontent" style="position:absolute; top:10%;">
			<div class="alert alert-info"><center><h2>Purchases	</h2></center></div>
			<br />
	...
	<label style="font-size:25px;">347 Inc.</label>
	<label style="font-size:20px;">Official Receipt</label>
		...
		<?php
		$pr_id = $_GET['prid'];
		$query = mysqli_query($conn, "SELECT * FROM purchase WHERE purchase_id = '$pr_id'") or die (mysqli_error());
		$fetch = mysqli_fetch_array($query);
		
		$amnt = $fetch['amount'];
		echo "Date : ". $fetch['order_date']."";
		
		$query2 = mysqli_query($conn, "SELECT * FROM purchase_detail LEFT JOIN product ON product.product_id = purchase_detail.product_id WHERE purchase_detail.purchase_id = '$pr_id'") or die (mysqli_error());
